...
//dbconn
    ...
    $sqlheader = "INSERT INTO patient_record_header(referredby, name, date, study, indication, rkey, attendedby, report_type, status) VALUES ('$referred','$pname', CURDATE(),'$study','$indication','$reportkey','$user','ogone','notPrinted')";
    $sql = "INSERT INTO obfirst(rkey, lmp, ga_lmp, eod_lmp, ga_usg,  crl, ges_week, ges_day, ro, rom, lo, lom) VALUES ('$reportkey',DATE '$lmp','$lmpga',DATE '$lmpeod','$usgga',DATE ,'$crl','$wk','$days','$ro','$rom','$lo','$lom')";
...
//dbconn
   ...
   $sql = "INSERT INTO obtwo(bpd, hc, ac, fl, placenta, liquor, afi, fhr, fwt, mi, geswk, gesdays, cl, rkey, bpdwd, hcwd, acwd, flwd, sdate) VALUES ('$bpd','$hc','$ac', '$fl','$placenta', '$liquor','$afi','$fhr', '$fwt','$minfo','$geswks','$gesdays','$cetvical','$reportkey',  '$bpdwd', '$hcwd', '$acwd', '$flwd', '$sdte')";
   if ($conn->query($sqstatusupdate) === TRUE) {
     if ($conn->query($sqlheader) === TRUE) {
     }
     if ($conn->query($sql) === TRUE) {
